...
clear all;
basis = ["H", "V", "D", "A", "L", "R"];
pa_1__port = serialport('COM9', 115200);         % Alice PA Setting the Baud Rate
...
for f=bandwidth_range
    ...
    for d1=1:6
        for d2=1:6
            coinc_mat(d1, d2) = measure_basis(pa_1__port, basis(d1), basis(d2));    
        end
    end
    %coinc_mat = measure_basis(pa_1__port, "H", "H");
    coinc_vec = reshape(coinc_mat, 1, []);      % A row vector of 36 coincidence counts
    display(coinc_vec); 
    file = fopen('Bob_frequency_tracking_coincidence_counts.txt', 'a');
    fprintf(file, f, coinc_vec);
end

function Coinc = measure_basis(port_1, x, y)
    ...
    input = ['Sending_Data_eth.py', ' ', '2'];
    pyrunfile(input);
    pause(2);

    input = ['Sending_Data_eth.py', ' ', 'c'];
    pyrunfile(input);
    pause(2);

    input = ['Sending_Data_eth.py', ' ', num2str(y)];
    disp(input);
    pyrunfile(input)    ;
    pause(2);
    ...
    % start_coinc_bin = 11; % start coincidence sum offset
    % window = 100;
    % end_coinc_bin = start_coinc_bin + (window); % end coincidence sum offset
    
    %%%% Fiber Loop
    Start_offset = -185500; % start histogram offset
    end_offset = -184500; % end histogram offset
    start_coinc_bin = 500; % start coincidence sum offset
    window = 41;
    end_coinc_bin = start_coinc_bin + (window); % end coincidence sum offset
    %
    sp = ' '; % no change
    ...
    site2name = 'b'; % b: Bob
    site1ip = '10.0.3.6'; % Alice IP address    
    site2ip = '10.0.3.7'; % Bob IP address 
