...
class CustomEPCOptimizer:
    ...
    def MATT(self, initial_voltages):
        ...
        # Define a threshold for switching from coarse to fine adjustments.
        cost_threshold = 0.1  
        print(f"Initial cost: {current_cost:.5f}")

        for i in range(self.max_iterations):
            # Coarse phase: when current cost is above the threshold.
            if current_cost > cost_threshold:
                # Dynamic step size based on current cost.
                ...
                # Fine phase: use a fixed small step size.
                step_size = 5

            print(f"Iteration {i}: current_cost = {current_cost:.5f}, step_size = {step_size:.2f}")

            best_cost_this_iter = current_cost
            best_voltages_this_iter = current_voltages.copy()

            # Try adjusting each voltage channel one at a time.
            for j in range(len(current_voltages)):
                perturbed_voltages = current_voltages.copy()
                perturbed_voltages[j] += step_size
                # Ensure the voltage remains within the allowed range.
                ...
                if perturbed_cost < best_cost_this_iter:
                    ...
                    best_voltages_this_iter = perturbed_voltages.copy()

            # If an improvement is found, update the current voltages.
            if best_cost_this_iter < current_cost:
                current_voltages = best_voltages_this_iter.copy()
                current_cost = self.compute_cost_record(current_voltages)
                print(f"Iteration {i}: Improvement found. New cost = {current_cost:.5f}")
            else:
                # In fine phase, if no improvement, apply a small random perturbation.
                if current_cost <= cost_threshold:
                    print(f"Iteration {i}: Fine phase - no improvement; applying small random perturbation.")
                    perturb = np.random.uniform(-step_size, step_size, size=current_voltages.shape)
                    current_voltages += perturb
                    current_voltages = np.array([wrap_voltage(v) for v in current_voltages])
                    current_cost = self.compute_cost_record(current_voltages)
                    print(f"Iteration {i}: After perturbation, cost = {current_cost:.5f}")
                else:
                    # In coarse phase, if no improvement is found, apply a minimal perturbation.
                    print(f"Iteration {i}: Coarse phase - no improvement; applying minimal perturbation.")
                    perturb = np.random.uniform(-1, 1, size=current_voltages.shape)
                    current_voltages += perturb
                    current_voltages = np.array([wrap_voltage(v) for v in current_voltages])
                    current_cost = self.compute_cost_record(current_voltages)
                    print(f"Iteration {i}: After minimal perturbation, cost = {current_cost:.5f}")

        print(f"Final Voltages: {current_voltages}, Final Cost: {current_cost:.5f}, Fidelity: {self.fidelity[-1]:.5f}")
