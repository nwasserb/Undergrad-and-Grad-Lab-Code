    ...
    def custom_optimization(self, initial_voltages):
        ...
        for i in range(self.max_iterations):
            ...
            if  self.cost and current_cost >= .1 and i >= 2 and abs(self.cost[-1] - self.cost[-2]) < 1e-3: #changed from 1e -5
                ...
                perturbed_cost_best = current_cost
                perturbed_voltages_best = current_voltages
                for j in range(len(current_voltages)):
                    
                    perturbed_voltages_up = np.copy(current_voltages)
                    perturbed_voltages_up[j] += step_size
                    perturbed_voltages_down = np.copy(current_voltages)
                    perturbed_voltages_down[j] -= step_size

                    perturbed_voltages_up = np.array([wrap_voltage(v) for v in perturbed_voltages_up])
                    perturbed_voltages_down = np.array([wrap_voltage(v) for v in perturbed_voltages_down])

                    perturbed_cost_up = self.compute_cost(perturbed_voltages_up)
                    perturbed_cost_down = self.compute_cost(perturbed_voltages_down)



                    if perturbed_cost_up or perturbed_cost_down < current_cost:
                        if perturbed_cost_up < perturbed_cost_down:
                            if perturbed_cost_up < perturbed_cost_best:
                                perturbed_cost_best = perturbed_cost_up
                                perturbed_voltages_best = perturbed_voltages_up
                        else:
                            if perturbed_cost_down < perturbed_cost_best:
                                perturbed_cost_best = perturbed_cost_down
                                perturbed_voltages_best = perturbed_voltages_down
                            
                current_cost = perturbed_cost_best
                current_voltages = perturbed_voltages_best

            sleep(.25) # added extra sleep to account for current best cost statement not aligning with actual best cost
            current_cost = self.compute_cost_record(current_voltages)
...
def main():
    ...
    #INITIAL_VOLTAGES = [ 585.90423918, -608.91312276,  446.13132941,   35.51490423]
    print(INITIAL_VOLTAGES)
    MAX_ITERATIONS = 800 #Value to change for number of iterations
    ...
    with open(name, 'w', newline='') as file:
            ...
            for i in range(len(epc_optimizer.cost)):
                data = np.concatenate[epc_optimizer.horizontal_power[i], epc_optimizer.diagonal_power[i], [epc_optimizer.cost[i], epc_optimizer.fidelity[i]], epc_optimizer.all_voltages[i]]
