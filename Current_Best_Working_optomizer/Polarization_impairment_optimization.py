...
from custom_optimizer_algorithm import MATT # Import custom algorithm
...
class CustomEPCOptimizer:
    ...
    def custom_optimization(self, initial_voltages):
        ...
        optimizer = MATT(
            ...
            initial_temperature=6_000, # adjust this, higher is slower but explores more of solution space
            cooling_rate=0.8, # the rate at which the probability of a guess decreases, closer to 1 is slower rate
            min_temperature=1e-3,
            adaptive=True # use adaptive temp system?
        )
        # optimizer = TNC(maxiter=self.max_iterations, disp=True, accuracy=0, ftol=- 1, xtol=- 1, gtol=- 1, tol=None, eps=1e-03)
        # bingus = TNC.minimize(fun = cost_function,
        #                     x0 = [random.randint(-5000,5000) for _ in range(4)],
        #                     # args=(),
        #                     method='TNC',
        #                     jac=None, 
        #                     # hess=None, 
        ...
        # best_voltages, best_cost = bingus
        # print(f"Final Voltages after Custom Optimization: {best_voltages}, Final Cost: {best_cost:.5f}, Fidelity: {self.fidelity[-1]:.5f}")
...
def main():
    ...
    MAX_ITERATIONS = 100 #Value to change for number of iterations
    # Time taken for 50 iterations: ~03:15
